<div class="page-container">
    ...
    <div class="page-content">
        ...
        <div class="page-title">                    
            <h2><span class="fa fa-users"></span> Formações</h2>
        </div>
                    ...
                    <div class="panel panel-default">
                        <div class="panel-body">
                            <div class="table-responsive">
                                <form action="<?=base_url()?>dados/inserirFormacao" method="post">
                                    <div class="row" style="margin-bottom: 10px">
                                        <div class="col-md-12">
                                            <div class="col-md-3 form-group"><label for="nome"><h3>Adicionar Formação:</h3></label></div>
                                ...
                                <table class="table datatable table-striped table-condensed table-actions">
                                    ...
                                    <tbody>
                                        <?php foreach($formacoes as $formacao) { ?>
                                            <tr id="formacao_<?=$formacao->id?>">
                                                <td><?=$formacao->nome?></td>                                                 
                                                <td>
                                                    <button class="btn btn-warning btn-condensed" onclick="alterarModal(<?=$formacao->id?>)"><i class="fa fa-pencil"></i></button>
                                                    <button class="btn btn-danger btn-condensed" onclick="deleteConfirm(<?=$formacao->id?>)"><i class="fa fa-times"></i></button>
                                                </td>                                                    
                                            </tr>
                                        <?php } ?>
                                    </tbody>
...
<div id="alterarModal" class="modal fade">
    <div class="modal-dialog">
        <div class="modal-content">
            <form action="<?=base_url()?>dados/alterarFormacao" method="post">
...
<script type="text/javascript">
    ...
    function deleteConfirm(id) {
        noty({
            text: 'Realmente deseja excluir esta Formação ?',
            layout: 'topRight',
            buttons: [{addClass: 'btn btn-success btn-clean', text: 'Sim', onClick: function($noty) {
                $.ajax({
                    url: '<?=base_url()?>dados/deletarFormacao',
                    type: 'post',
                    data: {id: id},
                    success: function() {
                        $('#formacao_' + id).attr("class","hidden");
                    }
                });
                $noty.close();
                noty({text: 'Formação Apagada!', layout: 'topRight', type: 'success'});
                }},
