                    ...
                    <div class="panel panel-default">
                        <div class="panel-body">
                            <div class="table-responsive">
                                <form action="<?=base_url()?>dados/insertPremiacao" method="post">
                                    <div class="row">
                                        <div class="col-md-12">
                                        ...
                                        <div class="col-md-12">
                                            ...
                                            <div class="col-md-9 form-group"><select class="form-control" name="pai" id="pai">
                                                ...
                                                <?php foreach($premiacoes as $premiacao) { ?>
                                                    <?php if($premiacao->pai == 0) { ?>
                                                        <option value="<?=$premiacao->id?>"><?=$premiacao->nome?></option>
                                                    <?php } ?>
                                                <?php } ?>
                                            </select></div>
                                        </div>
                                ...
                                <table class="table datatable table-striped table-condensed table-actions">
                                    ...
                                    <tbody>
                                        <?php foreach($premiacoes as $premiacao) { ?>
                                            <tr id="area_<?=$premiacao->id?>">
                                                <td><?=$premiacao->nome?></td>
                                                <td><?=$premiacao->pai == 0 ? "<p class='text-warning'>Categoria</p>" : "<p class='text-info'>Premiação</p>" ?></td>                                               
                                                <td>
                                                    <button class="btn btn-warning btn-condensed" onclick="alterarModal(<?=$premiacao->id?>)"><i class="fa fa-pencil"></i></button>
                                                    <button class="btn btn-danger btn-condensed" onclick="deleteConfirm(<?=$premiacao->id?>,'<?=$premiacao->pai == 0 ? "Categoria" : "Premiação" ?>')"><i class="fa fa-times"></i></button>
                                                </td>                                                    
                                            </tr>
                                        <?php } ?>
                                    </tbody>
...
<div id="alterarModal" class="modal fade">
    <div class="modal-dialog">
        <div class="modal-content">
            <form action="<?=base_url()?>dados/alterarPremiacao" method="post">
    ...
    function deleteConfirm(id, text) {
        noty({
            ...
            buttons: [{addClass: 'btn btn-success btn-clean', text: 'Sim', onClick: function($noty) {
                $.ajax({
                    url: '<?=base_url()?>dados/deletarPremiacao',
                    type: 'post',
                    data: {id: id},
                    success: function() {
